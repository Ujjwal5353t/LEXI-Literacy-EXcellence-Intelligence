...
const deliverEmail = async (message: EmailMessage): Promise<void> => {
  await transporter.sendMail({
    from: process.env.GMAIL_USER || 'no-reply@lexi.local',
    ...message,
  });
};
...
export const sendConsentEmail = async (to: string, token: string, studentName: string): Promise<void> => {
  ...
  await sendEmail({
    to,
    subject: 'Parental consent needed for Lexi',
    text: [
      'Hello,',
      '',
      'Lexi is an educational reading platform that helps students practise and understand reading patterns.',
      '',
      studentName + "'s account needs your consent before voice recording can be used. Lexi retains the recorded audio to allow you to review " + studentName + "'s reading progress over time. You may request deletion of this data at any time by contacting Lexi support or withdrawing consent.",
      '',
...
export const sendConsentWithdrawalEmail = async (to: string, studentName: string): Promise<void> => {
  await sendEmail({
    to,
    subject: 'Consent withdrawn for ' + studentName + "'s Lexi account",
    text: [
      'Hello,',
      '',
      "Your consent for " + studentName + "'s Lexi account has been withdrawn.",
      ...
      '',
      'You can contact the school or Lexi support if you have questions.',
    ].join('\n'),
  });
};
...
export const sendDataDeletionEmail = async (to: string, studentName: string): Promise<void> => {
  await sendEmail({
    to,
    subject: 'Reading data deleted for ' + studentName + "'s Lexi account",
    text: [
      'Hello,',
      '',
      "The stored reading data for " + studentName + "'s Lexi account has been deleted following the consent withdrawal.",
...
export const sendPasswordResetEmail = async (to: string, token: string): Promise<void> => {
  ...
  await sendEmail({
    to,
    subject: 'Set your password for Lexi parent account',
    text: [
      'Hello,',
      '',
      'A Lexi parent account has been created for you using this email address.',
      '',
